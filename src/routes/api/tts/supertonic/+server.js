...
async function initializeTTS() {
	...
	console.log('[Supertonic] Initializing TTS system...');
...
export async function POST({ request }) {
	try {
		...
		console.log(`[Supertonic] Synthesizing: "${text}" with voice ${voice}`);

		const { ttsInstance, styles } = await initializeTTS();
		const { wav, duration } = await ttsInstance.call(text, styles[voice], 5, speed);

		console.log(
			`[Supertonic] Generated ${wav.length} samples (${(wav.length / ttsInstance.sampleRate).toFixed(2)}s)`
		);
		...
		console.error('[Supertonic] Synthesis error:', err);
